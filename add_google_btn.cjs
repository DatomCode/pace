...
function injectGoogle(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');
  
  // Find where to insert. Right before {/* Error banner */} or <form 
  if (content.includes('{/* Error banner */}')) {
    content = content.replace('{/* Error banner */}', googleBtnHtml + '\n\n          {/* Error banner */}');
  } else if (content.includes('<form ')) {
    content = content.replace('<form ', googleBtnHtml + '\n\n          <form ');
  }
...
injectGoogle('src/pages/public/LoginPage.tsx');
injectGoogle('src/pages/public/RegisterPage.tsx');
